-- Anular intento aunque client_session no coincida (p. ej. recarga / storage desincronizado).
-- Sigue limitado a (examen, alumno) con state = in_progress y exam_allows_student.
...
-- Lista de anulados: validar maestro por teacher_id explícito (más fiable que auth.uid() vía header).
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'duration_seconds', greatest(
          0,
          extract(epoch from (ea.updated_at - ea.created_at))::int
        )
      )
